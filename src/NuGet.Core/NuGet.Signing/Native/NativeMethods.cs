...
namespace NuGet.Signing
{
    internal static class NativeMethods
    {
        ...
        internal const int ERROR_INVALID_PASSWORD_HRESULT = unchecked((int)0x80070056);
        ...
        public static extern bool CryptRetrieveTimeStamp(
            ...
            byte[] pbData,
            uint cbData,
            out SafeCryptMemHandle ppTsContext,
            IntPtr ppTsSigner,
            IntPtr phStore);
    ...
    internal sealed class SafeCryptMsgHandle : SafeHandle
    {
        ...
        private SafeCryptMsgHandle() : base(IntPtr.Zero, ownsHandle: true) { }

        internal SafeCryptMsgHandle(IntPtr handle) : base(handle, ownsHandle: true)
        {
        }

        internal SafeCryptMsgHandle(IntPtr handle, bool ownsHandle) : base(handle, ownsHandle)
        {
        }
    ...
    internal sealed class SafeCryptMemHandle : SafeHandle
    {
        public override bool IsInvalid { get { return handle == IntPtr.Zero; } }

        private SafeCryptMemHandle() : base(IntPtr.Zero, ownsHandle: true) { }

        internal SafeCryptMemHandle(IntPtr handle) : base(handle, ownsHandle: true)
        {
        }

        internal SafeCryptMemHandle(IntPtr handle, bool ownsHandle) : base(handle, ownsHandle)
        {
        }

        internal static SafeCryptMemHandle InvalidHandle
        {
            get { return new SafeCryptMemHandle(IntPtr.Zero); }
        }
